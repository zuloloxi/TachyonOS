...
.more_select:
	pusha
	mov bl, DIALOG_BOX_INNER_COLOUR		; Black on white for option list box
	mov dl, 21
	mov dh, 6
	mov si, 38
	mov di, 22
...
.draw_black_bar:
	...
	mov bh, 0
	mov cx, 36
	mov bl, DIALOG_BOX_SELECT_COLOUR
	mov al, ' '
	int 10h
...
.draw_white_bar:
	...
	mov bh, 0
	mov cx, 36
	mov bl, DIALOG_BOX_INNER_COLOUR	
	mov al, ' '
	int 10h
...
.redbox:				; Loop to draw all lines of box
	...
	mov bh, 0
	mov cx, 55
	mov bl, DIALOG_BOX_OUTER_COLOUR		; White on red
	mov al, ' '
	int 10h
	popa
...
.one_button:
	mov bl, DIALOG_BOX_INNER_COLOUR		; Black on white
	mov dh, 14
	mov dl, 35
	mov si, 8
	mov di, 15
...
.two_button_wait:
	...
	mov bl, DIALOG_BOX_OUTER_COLOUR		; White on red for cancel button
	mov dh, 14
	mov dl, 42
	mov si, 9
	mov di, 15
...
.noleft:
	...
	mov bl, DIALOG_BOX_OUTER_COLOUR		; Black on white
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	...
	mov bl, DIALOG_BOX_INNER_COLOUR		; White on red for cancel button
	mov dh, 14
	mov dl, 43
	mov si, 8
	mov di, 15
